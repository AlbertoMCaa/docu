...
let UpdateUserPasswordController = class UpdateUserPasswordController {
    ...
    execute(request, response) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                if (!request.body) {
                    this.httpService.badRequest(response, "No se proporcionó cuerpo");
                    return;
                }
                this.userRepository.saveUserPassword(new Uuid_1.Uuid(request.body.id), new Name_1.Name(request.body.name));
                this.httpService.ok(response, "Password Updated Succesfully");
            }
            catch (error) {
