...
export class getUserController implements HttpController {
    ...
    async execute(request: Request, response: Response): Promise<void> {
        try { 
            ...
            if (!id) {
                this.httpService.badRequest(response, { message: "No se proporcionó un Token válido" });
                return;
            }
            ...
            this.httpService.badRequest(response, {response: "No se proporcionó un Token válido"});
        }
    }
}